                                ...
                                {product.map((x) => (
                                    <Slide index={x.productid} key={x.productid} className="slide hover:bg-secondary hover:bg-opacity-20">
                                        <Link to={`../product/${x.productid}`} key={x.productid}>
                                            <div className="max-w-2xl max-h-2xl p-2 relative">
                                                ...
                                                <p className="text-xs text-alered font-bold sm:text-sm md:text-md lg:text-2xl">
                                                    {(((100 - x.sale) / 100) * x.price).toFixed(2)} € ALE-HINTA!
                                                </p>
                                                <p className="text-xs font-bold sm:text-sm md:text-md lg:text-2xl line-through">{x.price} €</p>
                                                <ButtonBack
                                                    role="button"
                                                    aria-label="slide backward"
                                                    className="absolute z-30 top-1/2 left-0 ml-8 -translate-y-1/2 focus:outline-none focus:bg-gray-400 focus:ring-2 focus:ring-offset-2 focus:ring-gray-400 cursor-pointer"
                                                    id="prev"
                                                >
                                                ...
                                                <ButtonNext
                                                    ...
                                                    className="absolute z-30 top-1/2 right-0 mr-8 -translate-y-1/2 focus:outline-none focus:bg-gray-400 focus:ring-2 focus:ring-offset-2 focus:ring-gray-400"
                                                    id="next"
                                                >
                                                    <svg width={8} height={14} viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
